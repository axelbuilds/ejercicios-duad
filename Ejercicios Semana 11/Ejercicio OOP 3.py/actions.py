...
class Students: #Constructor
    def __init__(self, name, section, spanish, english, history, science):
        ...
        self.sciente = science


    def calculate_average(self): #Function to calculate average scores
        return (self.spanish + self.english + self.history + self.science) / 4


    def object_to_dictionary(self): #Function to change object to dictionary for CSV export
        return {
            "Nombre": self.name,
            "Seccion": self.section,
            "Español": self.spanish,
            "Ingles": self.english,
            "Estudios Sociales": self.history,
            "Ciencias": self.science
        }

    def student_list(data): #Function to create objects from a dictionary
        return {
            data["Nombre"],
            data["Seccion"],
            data["Español"],
            data["Ingles"],
            data["Estudios Sociales"],
            data["Ciencias":]
        }
